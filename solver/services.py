...
def _try_real_world_problem(original: str) -> SolverResult | None:
    lower = original.lower()
    number_matches = list(re.finditer(r"-?\d+(?:\.\d+)?", original))
    numbers = [float(match.group()) for match in number_matches]
    if len(numbers) < 2:
        return None

    if any(word in lower for word in ["discount", "sale", "off"]):
        percent = _percent_value(original, numbers, number_matches)
        price = _largest_other_number(numbers, percent)
    ...
    if any(word in lower for word in ["interest", "loan", "investment"]):
        rate = _percent_value(original, numbers, number_matches)
        years = _duration_years(original, numbers, number_matches)
        principal = _largest_other_number(numbers, rate, years)
    ...
    if any(word in lower for word in ["speed", "distance", "time"]):
        ...
        asks_for_speed = any(phrase in lower for phrase in ["speed", "how fast", "rate"])
        asks_for_distance = any(phrase in lower for phrase in ["distance", "how far"])
        asks_for_time = any(phrase in lower for phrase in ["time", "how long"])

        if asks_for_speed:
            value = first / second
            label = "speed"
            formula = "speed = distance / time"
        elif asks_for_distance:
            value = first * second
            label = "distance"
...
def _percent_value(original: str, numbers: list[float], matches: list[re.Match]) -> float:
    for match in matches:
        after = original[match.end() : match.end() + 14].lower()
        if "%" in after or "percent" in after:
            return float(match.group())
    return numbers[1] if len(numbers) > 1 else numbers[0]


def _duration_years(original: str, numbers: list[float], matches: list[re.Match]) -> float:
    for match in matches:
        after = original[match.end() : match.end() + 12].lower()
        if "year" in after:
            return float(match.group())
    return numbers[2] if len(numbers) > 2 else 1


def _largest_other_number(numbers: list[float], *excluded: float) -> float:
    remaining = list(numbers)
    for value in excluded:
        for index, candidate in enumerate(remaining):
            if math.isclose(candidate, value):
                remaining.pop(index)
                break
    return max(remaining) if remaining else numbers[0]
